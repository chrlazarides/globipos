...
fn cfg_err() -> String {
    "Terminal not configured".to_string()
}
...
) -> Result<Option<TerminalConfig>, String> {
...
#[tauri::command]
async fn get_order_lines(
    state: State<'_, AppState>,
    order_id: String,
) -> Result<Vec<Value>, String> {
    let rows = sqlx::query("SELECT * FROM pos_order_lines WHERE order_id = ? ORDER BY sort_order")
        .bind(&order_id)
        .fetch_all(&state.db)
        .await
        .map_err(|e| e.to_string())?;
...
async fn get_customer_live(
        ...
        .await
    {
        ...
        Err(_) => match fallback.as_str() {
            ...
            "block_with_message" => {
                Err("Customer lookup requires server. Proceed without customer?".into())
            }
            _ => Ok(None),
        },
    }
...
async fn mark_inbox_processed(state: State<'_, AppState>, id: String) -> Result<(), String> {
    sqlx::query("UPDATE pos_inbox SET processed=1, processed_at=datetime('now') WHERE id=?")
        .bind(&id)
        .execute(&state.db)
        .await
        .map_err(|e| e.to_string())?;
...
async fn set_sco_heartbeat_state(
    ...
    sqlx::query(
        "INSERT OR REPLACE INTO schema_meta (key, value) VALUES ('sco_heartbeat_state', ?)",
    )
...
async fn send_heartbeat(app: AppHandle, state: State<'_, AppState>) -> Result<Value, String> {
    ...
    let shift_open: bool =
        sqlx::query("SELECT COUNT(*) as c FROM pos_shifts WHERE status = 'open'")
            .fetch_one(&state.db)
            .await
            .ok()
    ...
    let payment_cfg: PaymentConfig =
        sqlx::query("SELECT value FROM schema_meta WHERE key = 'payment_config'")
            .fetch_optional(&state.db)
            .await
            .ok()
            .flatten()
    ...
    let mut peripheral_status =
        hardware::build_peripheral_status(&app, &hw_cfg, &payment_cfg, cashier_name, shift_open)
            .await;
    if let Some(sco_state) =
        sqlx::query("SELECT value FROM schema_meta WHERE key = 'sco_heartbeat_state'")
            .fetch_optional(&state.db)
            .await
            .ok()
            .flatten()
...
#[tauri::command]
async fn get_shift_summary(state: State<'_, AppState>, shift_id: String) -> Result<Value, String> {
    ...
    let events: Vec<Value> =
        sqlx::query("SELECT * FROM shift_events WHERE shift_id = ? ORDER BY created_at")
            .bind(&shift_id)
            .fetch_all(&state.db)
            .await
            .map_err(|e| e.to_string())?
    ...
    let opening_float = shift
        .get("opening_float")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    let cash_sales = shift
        .get("total_cash_sales")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    let total_sales = shift
        .get("total_sales")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    let card_sales = shift
        .get("total_card_sales")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    let order_count = shift
        .get("order_count")
        .and_then(|v| v.as_i64())
        .unwrap_or(0);

    let cash_in: f64 = events
        .iter()
        .filter(|e| e.get("event_type").and_then(|v| v.as_str()) == Some("cash_in"))
        .filter_map(|e| e.get("amount").and_then(|v| v.as_f64()))
        .sum();
    let cash_out: f64 = events
        .iter()
    ...
    let avg_basket = if order_count > 0 {
        total_sales / order_count as f64
    } else {
        0.0
    };
    let top_payment = if cash_sales >= card_sales {
        "cash"
    } else {
        "card"
    };
...
async fn close_shift(
    ...
    let expected_cash = summary
        .get("expected_cash")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    let effective_closing = if closing_cash < 0.0 {
        expected_cash
    } else {
        closing_cash
    };
...
#[tauri::command]
async fn validate_coupon(state: State<'_, AppState>, code: String) -> Result<Value, String> {
    ...
    if let Some(r) = row {
        return Ok(
            serde_json::json!({ "valid": true, "promo": row_to_json(r), "message": "Coupon applied" }),
        );
    }
    ...
    let (srv, tc) = {
        ...
        match cfg.as_ref() {
            ...
            None => {
                return Ok(
                    serde_json::json!({ "valid": false, "message": "Offline — coupon not found" }),
                )
            }
        }
    };
...
async fn get_order_by_number(
    ...
    let order_row = sqlx::query("SELECT * FROM pos_orders WHERE order_number = ? LIMIT 1")
        .bind(&order_number)
        .fetch_optional(&state.db)
        .await
        .map_err(|e| e.to_string())?;
    ...
    if let Some(row) = order_row {
        ...
        let order_id = order
            ...
            .to_string();
        let lines: Vec<Value> =
            sqlx::query("SELECT * FROM pos_order_lines WHERE order_id = ? ORDER BY sort_order")
                .bind(&order_id)
                .fetch_all(&state.db)
                .await
                .map_err(|e| e.to_string())?
        ...
        result
            .as_object_mut()
            .unwrap()
            .insert("lines".to_string(), serde_json::json!(lines));
        Ok(Some(result))
    } else {
        Ok(None)
    }
...
async fn save_return_order(
    ...
    for line in &lines {
        ...
        .bind(&id)
        .bind(
            line.get("original_order_id")
                .and_then(|v| v.as_str())
                .unwrap_or(""),
        )
        .bind(
            line.get("original_line_id")
                .and_then(|v| v.as_str())
                .unwrap_or(""),
        )
    ...
    let outbox_id = uuid::Uuid::new_v4().to_string();
    let payload =
        serde_json::json!({ "type": "return_order", "return_order": return_order, "lines": lines });
    sqlx::query(
        "INSERT INTO pos_outbox (id, order_id, payload, status) VALUES (?, ?, ?, 'pending')",
    )
    .bind(&outbox_id)
...
async fn issue_credit_note(
    ...
    let outbox_id = uuid::Uuid::new_v4().to_string();
    let payload =
        serde_json::json!({ "type": "credit_note_issued", "credit_note": note_json.clone() });
    sqlx::query(
        "INSERT INTO pos_outbox (id, order_id, payload, status) VALUES (?, ?, ?, 'pending')",
    )
    .bind(&outbox_id)
...
async fn redeem_credit_note(
    ...
    let new_status = if new_remaining <= 0.001 {
        "redeemed"
    } else {
        "open"
    };
...
async fn issue_gift_voucher(
    ...
    let outbox_id = uuid::Uuid::new_v4().to_string();
    let payload =
        serde_json::json!({ "type": "gift_voucher_issued", "gift_voucher": voucher_json.clone() });
    sqlx::query(
        "INSERT INTO pos_outbox (id, order_id, payload, status) VALUES (?, ?, ?, 'pending')",
    )
    .bind(&outbox_id)
...
#[tauri::command]
async fn scale_tare(app: AppHandle, state: State<'_, AppState>) -> Result<(), String> {
    let cfg = hardware::load_hardware_config(&state.db).await;
...
#[tauri::command]
async fn open_cash_drawer(app: AppHandle, state: State<'_, AppState>) -> Result<(), String> {
    let cfg = hardware::load_hardware_config(&state.db).await;
    hardware::open_cash_drawer(&app, &cfg).await
}

#[tauri::command]
async fn check_printer_status(app: AppHandle, state: State<'_, AppState>) -> Result<bool, String> {
    let cfg = hardware::load_hardware_config(&state.db).await;
...
async fn process_card_payment(
    ...
    let result = hardware::process_payment(&payment_cfg, amount, &currency)
        .await
