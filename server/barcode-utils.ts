// Deterministic EAN-13-compatible barcode synthesis for textile-style color/size variant matrices.
// Layout (12 data digits + 1 check digit = 13 total, scannable as EAN-13/Code128/Code39):
//   [prefix:2]="29" (internal-use range) + [itemSeq:5] + [colorIndex:2] + [sizeIndex:2] + [seasonDigit:1]
...
export function generateVariantBarcode(params: {
  ...
  const itemPart = String((params.itemSequenceNo + (params.salt || 0) * 100000) % 100000).padStart(5, "0");
  ...
  const data12 = `29${itemPart}${colorPart}${sizePart}${seasonPart}`;
