...
import type { Category, Color, Size, InventoryInLine } from "@shared/schema";
...
export default function InventoryInPage() {
  ...
  const appendMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await apiRequest("POST", "/api/inventory-in", {
        ...
        costPrice, price1, vatRate, season: season || null, codeMethod, cells,
      });
  ...
  const postMutation = useMutation({
    ...
    onSuccess: (result: { posted: number; itemsCreated: number; variantsCreated: number }) => {
      queryClient.invalidateQueries({ queryKey: ["/api/inventory-in"] });
    ...
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <Card>
        <CardHeader className="p-3">
          <CardTitle className="text-sm">1. Style header</CardTitle>
        </CardHeader>
