...
**Security audit boundary:** Even on current Tauri 2 releases, the Linux target dependency graph can retain GTK 3's `glib` 0.18 line plus unmaintained `unic-*` parser crates and `proc-macro-error`. These cannot be upgraded independently because Tauri's Linux runtime constrains their major versions. Remove unused direct features (for example, SQLx database backends) and update independently fixable crates, but do not force incompatible GTK crate versions or suppress the residual advisories.

**Why:** Forcing `glib` 0.20 into the GTK 3 graph breaks semver/API compatibility, while replacing Tauri just to make a lockfile scanner reach zero would be a major product rewrite with greater release risk.
