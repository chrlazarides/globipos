...
const icons = {
  ...
  Refund: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8">
      <path d="M12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2ZM12 4C7.58172 4 4 7.58172 4 12C4 16.4183 7.58172 20 12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4ZM10 7L16 12L10 17V13H6V11H10V7Z" style={{transformOrigin: 'center', transform: 'rotate(180deg)'}} />
    </svg>
  ),
  Print: (
    <svg viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8">
...
const actions = [
  { name: 'Cash', color: 'bg-[#15803d]' },
  { name: 'Card', color: 'bg-[#15803d]' },
  { name: 'Voucher', color: 'bg-[#a8203c]' },
  { name: 'Loyalty', color: 'bg-[#a8203c]' },
  { name: 'Discount', color: 'bg-[#a8203c]' },
  { name: 'Hold', color: 'bg-gray-600' },
  { name: 'Recall', color: 'bg-gray-600' },
  { name: 'Void', color: 'bg-[#f87171]' },
  { name: 'Refund', color: 'bg-[#fbbf24]' },
  { name: 'Print', color: 'bg-gray-600' },
  { name: 'Subtotal', color: 'bg-[#a8203c]' },
  { name: 'Pay', color: 'bg-[#15803d]' },
];

export default function BoldSolid() {
  return (
    <div className="min-h-screen bg-[#030712] p-8 text-white font-sans selection:bg-white/10">
      <div className="max-w-4xl mx-auto space-y-10">
        <header className="space-y-2 border-b border-gray-800 pb-8">
          <h1 className="text-3xl font-semibold tracking-tight text-white">Bold Solid</h1>
          <p className="text-gray-400 text-lg">
            Fully filled, high-contrast shapes sitting on a color-coded square chip with gentle 16px corner radii and a subtle inner highlight for tactile depth.
          </p>
        </header>

        <div className="p-8 rounded-2xl bg-[#111827] border border-gray-800 shadow-2xl">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {actions.map((action) => {
              const Icon = icons[action.name as keyof typeof icons];
              return (
                <div key={action.name} className="flex flex-col items-center gap-4">
                  <button 
                    className={`
                      w-24 h-24 rounded-[16px] flex items-center justify-center
